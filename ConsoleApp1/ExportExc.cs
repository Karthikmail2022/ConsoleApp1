...
namespace ConsoleApp1
{
    ...
    public class ExportExc
    {
        ...
        public void CreateExcelFile(List<List<TestModel>> data, string OutPutFileDirectory)
        {
        ...
        private void CreatePartsForExcel(SpreadsheetDocument document, List<List<TestModel>> GroupedList)
        {
            WorkbookPart workbookPart1 = document.AddWorkbookPart();
            var partSheetDataList = new List<SheetData>();
            ...
            foreach (var testModels in GroupedList)
            {
                ...
                //partSheetDataList.Add(GenerateSheetdataForDetails(data));
                WorksheetPart worksheetPart1 = workbookPart1.AddNewPart<WorksheetPart>();
                SheetData partSheetData = GenerateSheetdataForDetails(testModels);
                GenerateWorkbookPartContent("ND-"+ sheetName, sheetId.ToString(),document.WorkbookPart.GetIdOfPart(worksheetPart1));                                
        ...
        private SheetData GenerateSheetdataForDetails(List<TestModel> data)
        {
            SheetData sheetData1 = new SheetData();
            sheetData1.Append(CreateHeaderRowForExcel());

            foreach (TestModel testmodel in data)
            {
        ...
        private void GenerateWorkbookPartContent(string name, string sheetId, string id)
        {
            //Workbook workbook1 = new Workbook();
            
            Sheet sheet1 = new Sheet() { Name = name, SheetId = Convert.ToUInt16(sheetId), Id = id };
        ...
        {
            Row tRow = new Row();
            tRow.Append(CreateCell(testmodel.StockLength.ToString()));
            tRow.Append(CreateCell(testmodel.Waste.ToString()));
            tRow.Append(CreateCell(testmodel.Material));
            tRow.Append(CreateCell(testmodel.Type));
            foreach (var keyValue in testmodel.Cuts)
            {
                tRow.Append(CreateCell(keyValue.Key.Cut + ";" + keyValue.Key.PosNumber + "-" + keyValue.Value));
            }
            return tRow;
        }
        ...
        {
            Row workRow = new Row();
            workRow.Append(CreateCell("Stock Length", 2U));
            workRow.Append(CreateCell("Waste", 2U));
            workRow.Append(CreateCell("Material", 2U));
            workRow.Append(CreateCell("Type", 2U));
